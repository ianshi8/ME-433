...
while True:
    ...
    vals = np.zeros(60,dtype=np.uint16)
    # threshold to try to find the line
    for i in range(0,60):
       if (reds[i]>62 and blues[i]>65 and greens[i]<35):
           bitmap[row,i] = 0xFFFF
           vals[i] = 1
       else:
           bitmap[row,i] = 0x0000
    ...
    #     numgreen += greens[i]*i
    #     demgreen += greens[i]
    # COMred = int(numred/demred)
    # COMblue = int(numblue/demblue)
    # COMgreen = int(numgreen/demgreen)
    # COM = int((COMred+COMblue+COMgreen)/3)
    # print("COM Red:", str(COMred))
    # print("COM Blue:", str(COMblue))
    # print("COM Green:", str(COMgreen))
    # print("COM", str(COM))

    # print red dot to COM
    bitmap[row, COM] = 0x3F<<5
